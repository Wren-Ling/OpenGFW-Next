...
func newWorker(config workerConfig) (*worker, error) {
	...
	tcpSF := &tcpStreamFactory{
		...
		MetadataEnricher: config.MetadataEnricher,
	}
...
func (w *worker) Run(ctx context.Context) {
	...
	for {
		...
		case wPkt := <-w.packetChan:
			...
			v, b := w.handle(wPkt.StreamID, wPkt.Packet, wPkt.Metadata)
...
func (w *worker) handle(streamID uint32, p gopacket.Packet, metadata map[string]string) (io.Verdict, []byte) {
	...
	case *layers.TCP:
		return w.handleTCP(ipFlow, p.Metadata(), tr, metadata), nil
	case *layers.UDP:
		v, modPayload := w.handleUDP(streamID, ipFlow, tr, metadata)
...
func (w *worker) handleTCP(ipFlow gopacket.Flow, pMeta *gopacket.PacketMetadata, tcp *layers.TCP, metadata map[string]string) io.Verdict {
	ctx := &tcpContext{
...
func (w *worker) handleUDP(streamID uint32, ipFlow gopacket.Flow, udp *layers.UDP, metadata map[string]string) (io.Verdict, []byte) {
	ctx := &udpContext{
